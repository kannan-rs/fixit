(function( $ ) {
  $.widget( "custom.combobox2", {
    _create: function() {
      ...
      if( this.element[0].id.indexOf("contractorcity") >= 0 ) {
        this._addCityEventsToInput();
        $( this.input ).attr("placeholder", "Enter first 3 letter of the city").val();
        $("#contractorcity").val();
        this._createError({id : "contractorcityError", "text" : ""});
      }
    },
    ...
    _addCityEventsToInput: function() {
        $( this.input ).on({
          keyup : function() { 
              ...
              $("#contractorcityError").css({"display" : "none"}).removeClass("form-error");
              _utils.getAndSetMatchCity(this.value,'', $(this).attr('id'));
          },
          blur : function() {
            if(this.value != $("#contractorcity").val()) {
                $("#contractorcity").val("");
            } else {
              $(this).removeClass("form-error").next().removeClass("form-error");
              $("#contractorcityError").css({"display" : "none"}).removeClass("form-error");
